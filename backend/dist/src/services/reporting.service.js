...
class ReportingService {
    ...
    static async getFilterOptions(user) {
        ...
        if (deptScope)
            courseWhere.departmentId = deptScope;
    ...
    static async getTeacherPerformanceReport(filters, user) {
        ...
        const teacherSummaries = await Promise.all(teacherEmployees.map(async (t) => {
            ...
            const pendingEvaluations = submissions.filter((s) => s.status === "SUBMITTED" && s.submissionType !== "FEEDBACK" && !s.submissionText?.includes('"type":"FEEDBACK"')).length;
    ...
    static async getQuizAssessmentReport(filters, user) {
        ...
        const activeCourses = await prisma_1.default.course.findMany({
            where: {
                isActive: true,
