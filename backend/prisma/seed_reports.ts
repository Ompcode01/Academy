import { PrismaClient, EmploymentStatus, CourseStatus } from "@prisma/client";
...
async function main() {
  ...
  for (let i = 0; i < allEmpList.length; i++) {
    ...
    for (let j = 0; j < numCourses; j++) {
      ...
      let startDate: Date | null = new Date(enrolledAt.getTime() + 86400000);
      let completedAt: Date | null = null;
      let dueDate: Date | null = new Date(enrolledAt.getTime() + 20 * 86400000);
      let lastActivityAt: Date | null = new Date(now.getTime() - (i % 15) * 86400000);
