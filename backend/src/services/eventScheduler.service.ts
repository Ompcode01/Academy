...
function parseEventStartDateTime(eventDate: Date, eventTime?: string | null): Date {
...
export async function checkAndDispatchEventReminders() {
  try {
    const now = new Date();
    ...
    const pendingEvents = await (prisma.event as any).findMany({
      where: {
        reminderSent: false,
        eventDate: {
          gte: todayStart,
